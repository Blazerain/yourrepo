...
# 创建安全的curl函数
safe_curl() {
    # 优先尝试正常curl，失败则使用-k参数
    curl "$@" 2>/dev/null || curl -k "$@" 2>/dev/null || {
        echo "网络请求失败，尝试使用HTTP..." >&2
        # 将https替换为http重试
        local args=("$@")
        for i in "${!args[@]}"; do
            args[$i]="${args[$i]//https:/http:}"
        done
        curl "${args[@]}" 2>/dev/null || return 1
    }
}
...
get_external_ip() {
    ...
    local external_ip=""
    # 使用安全curl函数，尝试多个IP检测服务
    for service in "ifconfig.me" "ipinfo.io/ip" "icanhazip.com" "ipecho.net/plain" "myip.dnsomatic.com"; do
        external_ip=$(timeout 10 safe_curl -s --max-time 8 "http://$service" 2>/dev/null | tr -d '\n\r' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+
...
if command -v yum >/dev/null 2>&1; then
    ...
    if ! command -v ss-server >/dev/null 2>&1; then
        ...
        if ! timeout 300 yum install -y shadowsocks-libev >/dev/null 2>&1; then
            ...
            # 尝试GitHub Release
            if safe_curl -L --timeout=30 -o shadowsocks-libev.tar.gz "https://github.com/shadowsocks/shadowsocks-libev/releases/download/v3.3.5/shadowsocks-libev-3.3.5.tar.gz"; then
                download_success=true
            # 备用下载地址
            elif safe_curl -L --timeout=30 -o shadowsocks-libev.tar.gz "http://github.com/shadowsocks/shadowsocks-libev/releases/download/v3.3.5/shadowsocks-libev-3.3.5.tar.gz"; then
                download_success=true
            fi
            ...
            if [ "$download_success" = true ]; then
                ...
                tar -xzf shadowsocks-libev.tar.gz 2>/dev/null || echo "  ⚠️ 解压可能有问题..."
...
if command -v ss-server >/dev/null 2>&1; then
    ...
    for url in \
        ...
        "http://github.com/shadowsocks/go-shadowsocks2/releases/download/v0.1.5/shadowsocks2-linux.gz"
    do
        if safe_curl -L --timeout=30 -o shadowsocks2-linux.gz "$url"; then
            download_success=true
            break
        fi
    done
    ...
    if [ "$download_success" = true ]; then
        gunzip shadowsocks2-linux.gz 2>/dev/null || echo "解压可能有问题"
        if [ -f shadowsocks2-linux ]; then
            ...
            echo "❌ shadowsocks安装失败，请检查网络或手动安装"
            exit 1
        fi
    else
        echo "❌ shadowsocks下载失败，请检查网络连接"
        exit 1
    fi
fi
...
echo ""
echo "🎊 Shadowsocks代理服务已就绪！" | head -1)
    ...
    for service in "ifconfig.me" "ipinfo.io/ip"; do
        external_ip=$(timeout 10 safe_curl -s --max-time 8 "https://$service" 2>/dev/null | tr -d '\n\r' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+
