...
echo -e "${GREEN}"
echo "========================================"
echo "   游戏代理一键安装脚本 (DNS工具增强版)"
echo "========================================"
echo -e "${NC}"
echo "功能包括："
echo "• 强制安装DNS解析工具 (dig/nslookup)"
echo "• 智能IP发现 + 内置备用IP列表"
echo "• 网络和DNS问题自动修复"
...
discover_game_ips() {
    ...
    builtin_ips["hk.beanfun.com"]="18.167.13.186"
    builtin_ips["csp.hk.beanfun.com"]="18.167.13.186"
    builtin_ips["bfweb.hk.beanfun.com"]="18.163.12.31"
    builtin_ips["tw.beanfun.com"]="54.230.123.123"
    builtin_ips["beanfun.com"]="54.230.123.124"
    builtin_ips["www.beanfun.com"]="54.230.123.124"
    
    if [ "$DNS_TOOL" = "builtin" ]; then
        log_warn "DNS工具不可用，使用内置IP列表..."
        ...
        VALID_IPS=($(printf '%s\n' "${!discovered_ips[@]}" | sort))  # 假设内置IP都有效
        
        log_info "使用内置IP列表: ${#DISCOVERED_IPS[@]} 个地址"
        return 0
    fi
    ...
    resolve_domain_ip() {
        ...
        if [ "$DNS_TOOL" = "dig" ]; then
            result=$(dig @$dns +short $domain A 2>/dev/null | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+
...
backup_ips["hk.beanfun.com"]="18.167.13.186"
backup_ips["csp.hk.beanfun.com"]="18.167.13.186"
backup_ips["tw.beanfun.com"]="54.230.123.123"
backup_ips["beanfun.com"]="54.230.123.124"
...
main "$@" | head -1)
            ...
            # 改进的nslookup解析逻辑
            result=$(nslookup $domain $dns 2>/dev/null | grep "Address:" | grep -v "$dns" | head -1 | awk '{print $2}' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+
...
main "$@")
            ...
            # 如果上面的方法失败，尝试另一种方式
            if [ -z "$result" ]; then
                result=$(nslookup $domain $dns 2>/dev/null | awk '/^Address: [0-9]/ {print $2}' | head -1)
            fi
        fi
